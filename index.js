const { closeTicket, reopenTicket, deleteTicket } = require('../utils/ticketManager');
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isButton()) {
      ...
      // Vérification : le salon est bien un ticket ?
      const isTicketChannel = interaction.channel?.topic?.startsWith('ticket:');

      if (customId === 'ticket_open') {
        // Géré dans la commande /ticket panel via select menu
        return;
      }
    ...
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'ticket_reason') {
        ...
        return createTicket(interaction, reason);
      }
    }
  },
};
